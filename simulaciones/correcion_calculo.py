...
"""
Paso 1: Sim GEANT4 + flujo inyectado tipo ARTI/CORSIKA
Objetivo:
  - Distribución angular cruda (sin corrección)
  - Corrección geométrica (N/G)
  - Corrección geométrica + eficiencia 2D (N/(G*eps_smooth))
  - Comparación con ARTI esperado: cos^n(theta), n=2.25 (por defecto)
  - Mapas 2D: N, G, N/G, eps_raw, eps_smooth, N/(G*eps_smooth)
  - Curvas 1D vs theta hasta theta_max (por defecto 60°) con barras Poisson

Modelo base:
  N(theta_x,theta_y) = K * G_geom(theta_x,theta_y) * cos^n(theta) * eps(theta_x,theta_y)

donde:
  G_geom = A_overlap * cos(theta) * dOmega
  A_overlap: solape de dos cuadrados (60x60 cm) desplazados por |Δx|p, |Δy|p
  dOmega: pixel en espacio de slopes (u,v), con u=tan(theta_x)=Δx*p/d

Uso:
  python3 paso1_corr_2D_arti.py mapa_pixeles_delta_xy_sim.csv --outdir out --distance-cm 30 --n-art 2.25
...
# -------------------------
# Utilidades: smoothing 2D ponderado (sin scipy)
# -------------------------
...
def gaussian_smooth_weighted(value: np.ndarray, weight: np.ndarray, sigma_pix: float) -> np.ndarray:
    """
    Suaviza value con un kernel gaussiano separable, ponderado por weight.
    Maneja NaN vía weight=0.
    """
    ...
    out = np.divide(num_s, w_s, out=np.full_like(num_s, np.nan), where=w_s > 0)
    return out
...
# Geometría y binning
# -------------------------
def build_geometry(max_delta: int, pitch_cm: float, distance_cm: float, nbars: int, n_art: float):
    L = nbars * pitch_cm
    ...
    phi = np.cos(np.radians(theta_deg)) ** n_art  # ARTI relativo

    # Edges para pcolormesh en (theta_x, theta_y)
...
def theta_binning(theta_grid: np.ndarray, weights_grid: np.ndarray, binw_deg: float):
    th = theta_grid.ravel()
    ...
    b = np.bincount(idx[valid], weights=ww[valid], minlength=len(centers))
    return centers, b


def fit_K(centers, N_bin, G_bin, n_art: float, theta_max_fit: float, min_counts_fit: float) -> float:
...
def first_valid_index(arr: np.ndarray):
...
def norm_by_first(arr: np.ndarray):
    i0 = first_valid_index(arr)
    if i0 is None:
        return arr, None
    return arr / arr[i0], i0
...
def parse_args():
    ...
    p.add_argument("--outdir", default="out_paso1_2D", help="Directorio de salida")
    ...
    p.add_argument("--n-art", type=float, default=2.25)
...
def main():
    ...
    geom = build_geometry(args.max_delta, args.pitch_cm, args.distance_cm, args.nbars, args.n_art)
    N_grid = fill_counts_grid(sim, geom["deltas"])

    # Binning 1D para ajustar K
    centers, N_bin = theta_binning(geom["theta_deg"], N_grid, args.theta_bin_deg)
    _, G_bin = theta_binning(geom["theta_deg"], geom["G"], args.theta_bin_deg)

    K = fit_K(centers, N_bin, G_bin, args.n_art, args.theta_max_fit, args.min_counts_fit)
    print(f"K (normalización global) = {K:.6g}")

    # -----------------------------
    # EPS RAW 2D: eps_raw = N/(K*G*cos^n)
    # -----------------------------
    ...
    # Relleno de NaNs con promedio por anillos r^2 (prior) para estabilizar bordes
    r2 = geom["DX"] * geom["DX"] + geom["DY"] * geom["DY"]
    eps_fill = eps_raw.copy()
    for r2_val in np.unique(r2):
        m = (r2 == r2_val) & np.isfinite(eps_raw)
        if np.any(m):
            ww = np.maximum(N_grid[m], 1.0)
            mu = float(np.sum(eps_raw[m] * ww) / np.sum(ww))
            eps_fill[(r2 == r2_val) & ~np.isfinite(eps_fill)] = mu

    # Suavizado 2D ponderado por estadística (N)
    w_eps = np.where(np.isfinite(eps_fill), np.maximum(N_grid, 1.0), 0.0)
    ...
    F_eff_grid = np.divide(
        N_grid, geom["G"] * eps_smooth,
        out=np.full_like(N_grid, np.nan),
        where=(geom["G"] > 0) & (eps_smooth > 0) & (N_grid > 0)
    )

    # -----------------------------
    # Curvas 1D (hasta theta_max_plot)
    #   - Crudo: N(θ) (normalizado)
    #   - Geom: (N/G)(θ) (normalizado)
    #   - Geom+2D: (N/(G*eps))(θ) (normalizado)
    # -----------------------------
    theta_max_plot = args.theta_max_plot

    # Crudo N(θ)
    centers, Nbin = theta_binning(geom["theta_deg"], N_grid, args.theta_bin_deg)
    sigN = np.sqrt(np.maximum(Nbin, 0.0))

    N_rel, i0 = norm_by_first(Nbin)
    if i0 is None:
        raise RuntimeError("No hay bins válidos para normalizar.")
    sigN_rel = prop_norm_error(Nbin, sigN, i0)

    # Predicción para crudo: N_pred(θ)=K*G(θ)*cos^n
    _, Gbin = theta_binning(geom["theta_deg"], geom["G"], args.theta_bin_deg)
    phi_bin = np.cos(np.radians(centers)) ** args.n_art
    Npred = K * Gbin * phi_bin
    Npred_rel = Npred / Npred[i0] if (np.isfinite(Npred[i0]) and Npred[i0] > 0) else Npred

    # Geom: F=N/G
    Fgeom = np.divide(Nbin, Gbin, out=np.full_like(Nbin, np.nan), where=Gbin > 0)
    sigFgeom = np.divide(sigN, Gbin, out=np.full_like(sigN, np.nan), where=Gbin > 0)
    Fgeom_rel = Fgeom / Fgeom[i0]
    sigFgeom_rel = prop_norm_error(Fgeom, sigFgeom, i0)

    # Geom+2D: F=N/(G*eps_smooth); binea denominador correctamente
    _, Geff_bin = theta_binning(geom["theta_deg"], geom["G"] * eps_smooth, args.theta_bin_deg)
    ...
    # Selección hasta theta_max_plot
    sel = centers <= theta_max_plot

    # -----------------------------
    # GRAFICAS 1D (las que tú quieres claras)
    # -----------------------------
    # (1) CRUDO: N(θ) + modelo K G cos^n  (con barras)
    ...
    plt.ylabel("Distribución angular cruda (normalizada)")
    plt.title(f"Crudo vs modelo (θ≤{theta_max_plot:g}°)")
    plt.legend()
    savefig(outdir / "1D_crudo_counts_vs_theta_con_modelo.png")

    # (2) GEOM: N/G vs cos^n (con barras)
    ...
    plt.xlim(0, theta_max_plot)
    plt.xlabel(r"Ángulo cenital $|\theta|$ [deg]")
    plt.ylabel("Distribución corregida geométricamente (normalizada)")
    plt.title(f"Corrección geométrica (θ≤{theta_max_plot:g}°)")
    plt.legend()
    savefig(outdir / "1D_correcion_geometrica_N_over_G_vs_costheta.png")

    # (3) GEOM + 2D: N/(G eps) vs cos^n (con barras)
    ...
    plt.errorbar(centers[m3], Feff_rel[m3], yerr=sigFeff_rel[m3], fmt="o", markersize=3, capsize=2,
                 label=r"Geom.+2D: $N/(G\,\varepsilon_{smooth})(\theta)$ (norm., Poisson)")
    plt.plot(centers[m3], Fart_rel[m3], "-", label=fr"ARTI esperado: $\cos^{{{args.n_art:.2f}}}\theta$ (norm.)")
    plt.yscale("log")
    plt.xlim(0, theta_max_plot)
    plt.xlabel(r"Ángulo cenital $|\theta|$ [deg]")
    plt.ylabel("Distribución corregida (normalizada)")
    plt.title(f"Corrección geométrica + eficiencia 2D (θ≤{theta_max_plot:g}°)")
    plt.legend()
    savefig(outdir / "1D_correcion_geom_mas_eficiencia2D_vs_costheta.png")

    # (4) Comparación en una sola figura: Geom vs Geom+2D vs ARTI (con barras)
    plt.figure(figsize=(7.6, 4.8))
    plt.errorbar(centers[m2], Fgeom_rel[m2], yerr=sigFgeom_rel[m2], fmt="o", markersize=3, capsize=2,
                 label=r"Solo geom.: $(N/G)(\theta)$ (norm.)")
    plt.errorbar(centers[m3], Feff_rel[m3], yerr=sigFeff_rel[m3], fmt="o", markersize=3, capsize=2,
                 label=r"Geom.+2D: $N/(G\varepsilon)(\theta)$ (norm.)")
    ...
    plt.legend()
    savefig(outdir / "1D_compare_geom_vs_2Deps_vs_ARTI.png")

    # -----------------------------
    # GRAFICAS 2D (las que ya te había dado)
    # -----------------------------
    ...
    plt.xlabel(r"$\theta_x$ [deg]")
    plt.ylabel(r"$\theta_y$ [deg]")
    plt.title(r"$\varepsilon_{\rm raw}=N/(K\,G\,\cos^{2.25}\theta)$")
    plt.gca().set_aspect("equal", adjustable="box")
    ...
    print(f"Listo. Figuras y salidas en: {outdir.resolve()}")

...
def build_pixel_map(csv_path: Path, chunk_size: int, max_delta: int):
    ...
    NOTA (importante): En algunas corridas el plano Y inferior puede estar cableado
    en sentido inverso respecto al plano Y superior. En ese caso, hay que corregir
    el índice de Y_bottom con:  idx4 = (N_BARRAS-1) - idx4  antes de calcular Δy.
    Este script aplica esa corrección (flip en Y_bottom) por defecto.
    ...
    for chunk in iterator:
        ...
        # FIX: el plano Y inferior (ch46..ch60) está invertido respecto al Y superior en estos datos.
        #      Corregimos el índice para que 'misma barra' corresponda a dy=0.
        n_bars = g4_idx.size  # 15
        idx4_valid = (n_bars - 1) - idx4_valid
...
def plot_angular_map(counts: np.ndarray, max_delta: int,
    ...
    counts_plot = np.ma.masked_less_equal(counts, 0)  # evita 0/negativos en log
    pcm = ax.pcolormesh(
    theta_x_edges,
    theta_y_edges,
    counts_plot,
